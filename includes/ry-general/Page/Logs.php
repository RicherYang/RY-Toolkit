...
final class Logs extends AbstractAdminPage
{
    ...
    public function output_page(): void
    {
        ...
        echo '<div class="wrap">';
        echo '<h1 class="wp-heading">' . esc_html__('Logs', 'ry-toolkit') . '</h1>';
        if ($current_group === '') {
            echo '<p>' . esc_html__('No logs found.', 'ry-toolkit') . '</p>';
            echo '</div>';
            return;
        }
        ...
        if (isset($this->log_list[$current_group], $this->log_list[$current_group][$current_log])) {
            ...
            if (str_starts_with($current_file, $this->log_path)) {
                if (wp_delete_file($current_file)) {
                    $this->add_notice('success', __('Delete successful.', 'ry-toolkit'));
                    $current_log = '';
                    ...
                    $this->add_notice('error', __('Delete failed.', 'ry-toolkit'));
                }
